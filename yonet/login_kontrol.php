...
if(isset($_POST['kadi']) && isset($_POST['sifre']))
{
	if ($_POST['kadi']!="" && $_POST['sifre']!="")
	{
		$kullanici=$db->prepare("
			...
			FROM kullanici WHERE kullanici_mail=? AND kullanici_sifre=? AND kullanici_yetki=1 ");
		$kullanici->execute(
			array(
				$_POST['kadi'],
				$_POST['sifre']
		...
		else
		{
			$kullanici=$db->prepare("
			SELECT  kullanici_id,kullanici_mail,kullanici_adsoyad 
			FROM kullanici WHERE kullanici_mail=? AND kullanici_sifreunuttum=? AND kullanici_yetki=1 ");
		$kullanici->execute(
			array(
				$_POST['kadi'],
				$_POST['sifre']
		...
		if($kullanici->rowCount()==1)
		{
			...
			$db->query(
				"UPDATE kullanici 
				SET kullanici_sifre=kullanici_sifreunuttum,kullanici_sifreunuttum='',
				kullanici_songiris=NOW()
				WHERE kullanici_id=".$kullanici['kullanici_id']);
